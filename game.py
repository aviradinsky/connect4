...
def inputHeuristic(s):
    ...
    for i in range(0, columns):  # this simple agent always plays min
        ...
        makeMove(tmp, i)
        if (
            value(tmp) < temp and s.board[0][i] == 0
        ):  # so a "loss" is a win for this side
            tmp_col = i
            temp = value(tmp)
    makeMove(s, tmp_col)
...
def inputMC(s):
    # Initialize a list to count the number of wins for each column
    l = [0 for _ in range(columns)]
    
    # Iterate through each column to simulate potential moves
    for m in range(columns):
        # Run 100 simulations for each column
        for _ in range(100):
            # Create a deep copy of the current game state
            co = copy.deepcopy(s)
            
            # Make a move in the current column for the simulation
            makeMove(co, m)
            
            # Continue making random moves until the game is finished
            while not isFinished(co):
                inputMRand(co)
            
            # If the simulation results in a victory, increment the win count for the current column
            l[m] += 1 if value(co) == VICTORY else 0
    
    # Determine the best move by selecting the column with the maximum number of simulated victories
    x = 0
